...
function distToColorStripe(
	...
	return (dist: number) =>
		Math.floor((dist + time) / stripeWidth) % 2 === 0 ? (dist > 0 ? inner1 : outer2) : dist > 0 ? inner2 : outer1;
}
...
function loop() {
	(render(objects, distToColorStripe(10, [255, 255, 128], [128, 128, 64], [128, 255, 255], [64, 128, 128])), requestAnimationFrame(loop));
	time++;
}
loop();
